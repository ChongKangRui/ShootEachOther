	...
	if (IsLocallyControlled()) {
		if (const UWeaponInstance* wi = GetEquippedWeaponInstance()) {
			if (const AShootEachOtherCharacter* owner = GetSEOCharacter()) {
				...
				FCollisionQueryParams TraceParams(SCENE_QUERY_STAT(MeleeTrace), true, GetAvatarActorFromActorInfo());
				...
				HandleDamageToEnemy(HitResults, data);
...
void USGA_MeleeSlash::HandleDamageToEnemy(TArray<FHitResult>& HitResults, const FWeaponData& Data)
{
